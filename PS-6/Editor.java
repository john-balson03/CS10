...
public class Editor extends JFrame {	
	 ...
	 */
	private synchronized JComponent setupCanvas() {
	...
	public void drawSketch(Graphics g) {
		// TODO: YOUR CODE HERE
		Iterator i = sketch.getShapeIDs().navigableKeySet().iterator();
		while(i.hasNext())
		{
			sketch.getShapeIDs().get((Integer)i.next()).draw(g);
		}
		if(mode == Mode.DRAW && curr != null) { curr.draw(g); }
	}
	...
	private void handlePress(Point p) {
		...
		else if(mode == Mode.MOVE)
		{
			...
			if(movingId != -1)
			{
				curr = sketch.getShapeIDs().get(movingId);
				moveFrom = new Point(p.x, p.y);
			}
		}
		else if(mode == Mode.RECOLOR)
		{
			Integer id = sketch.contains(p.x, p.y);
			if(id != -1)
			{
				comm.send(id+" recolor "+color.getRGB());
				System.out.println("shape "+id+" recolored");
			}
		}
	...
	private void handleDrag(Point p) {
		...
		if(mode == Mode.DRAW)
		{
			...
			{
				((Polyline)curr).addSegment(p.x, p.y);
			}
			else if(shapeType.equals("segment"))
			{
	...
	private void handleRelease(Point p) {
		...
		if(mode == Mode.DRAW)
		{
			...
			{
				comm.send("-1 "+curr);
				System.out.println("drew a(n) "+shapeType);
				repaint();
			}
		}
		else if(mode == Mode.MOVE)
		{
			...
			{
				comm.send(movingId+" "+curr);
				System.out.println("shape "+movingId+" moved");
				repaint();
			}
		}
	}
